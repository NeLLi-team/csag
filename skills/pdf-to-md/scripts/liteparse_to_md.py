...
HEADING_NUMBER_RE = re.compile(r"^(?:\d+(?:\.\d+)*[.)]?|[IVXLCDM]+[.)]|[A-Z][.)])\s+")
...
def to_markdown(pages, heading_keys: set[str], title_lines: list[str], drop_keys: set[str]) -> tuple[str, int]:
    out: list[str] = []
    paragraph: list[str] = []
    pending = ""  # word fragment before a line-end hyphen, joined to the next line
    ...
    for page in pages:
        for raw in page.text.splitlines():
            ...
            if line.endswith("-") and not line.endswith(" -"):
                pending += line[:-1]  # de-hyphenate across line breaks
            else:
